...
//        print_indiv(pop[best], 0);
    ...
    private char[] crossover(char[] parent1, char[] parent2) {
        ...
        // If the length would be too large then simply mutate parent1
        if (lenoff > MAX_XO) {
            return mutation(parent1, PMUT_PER_NODE);
        }
    ...
    private void evolve() {
        int gen, indivs, offspring, parent1, parent2;
        double newfit;
        char[] newind;
        print_parms();
        stats(fitness, pop, 0);
        for (gen = 1; gen < GENERATIONS; gen++) {
            ...
            for (indivs = 0; indivs < POPSIZE; indivs++) {
                ...
                char[] parent1Arr = pop[parent1];
                if (rd.nextDouble() < CROSSOVER_PROB) {
                    parent2 = tournament(fitness, TSIZE);
                    newind = crossover(parent1Arr, pop[parent2]);
                } else {
                    newind = mutation(parent1Arr, PMUT_PER_NODE);
                }
